...
if [ ! -f "$INSTALL_DIR/.step1-base-packages" ]; then
    ...
    if ! curl -fsSL https://ollama.com/install.sh | OLLAMA_VERSION=0.17.3 sh > /var/log/ollama-install.log 2>&1; then
...
# Step 4: Install integrity monitoring + idle monitor
if [ ! -f "$INSTALL_DIR/.step4-integrity-monitor" ]; then
    echo "[STEP 4] Installing integrity monitoring + idle monitor..."
...
EOF
    ...
    # Integrity monitor systemd service
...
EOF
    ...
    # Integrity monitor timer (every 5 minutes)
...
EOF
    ...
    # Create idle monitor script (self-shutdown when idle)
    sudo tee /usr/local/bin/idle-monitor.sh > /dev/null <<'EOF'
#!/bin/bash
IDLE_TIMEOUT=$(curl -sf -H "Metadata-Flavor: Google" http://metadata.google.internal/computeMetadata/v1/instance/attributes/idle-timeout)
IDLE_TIMEOUT=${IDLE_TIMEOUT:-300}
NOW=$(date +%s)

# Honeymoon: first-ever boot gets 30 min grace for package installation
# Creation time is fetched once per boot and cached in tmpfs
CREATED_CACHE="/var/run/private-llm-vm-created"
if [ ! -f "$CREATED_CACHE" ]; then
    ...
    gcloud compute instances describe "$NAME" --zone="$ZONE" --format='value(creationTimestamp)' > "$CREATED_CACHE" 2>/dev/null
fi
CREATED=$(cat "$CREATED_CACHE" 2>/dev/null)
if [ -n "$CREATED" ]; then
    CREATED_EPOCH=$(date -d "$CREATED" +%s 2>/dev/null)
    if [ -n "$CREATED_EPOCH" ] && [ $((NOW - CREATED_EPOCH)) -lt 1800 ]; then
        exit 0
    fi
fi

# Check idle time from Caddy access log
LOG="/var/log/caddy/access.log"
if [ -f "$LOG" ]; then
    LAST=$(stat -c %Y "$LOG")
else
    # No requests yet — measure from boot time
    LAST=$((NOW - $(awk '{print int($1)}' /proc/uptime)))
fi
IDLE=$((NOW - LAST))
[ "$IDLE" -ge "$IDLE_TIMEOUT" ] && shutdown -h now
EOF

    sudo chmod +x /usr/local/bin/idle-monitor.sh

    # Idle monitor systemd service
    sudo tee /etc/systemd/system/idle-monitor.service > /dev/null <<'EOF'
[Unit]
Description=Private LLM Idle Monitor (auto-shutdown)
After=network.target
...
EOF
    ...
    # Idle monitor timer (every 1 minute, starting 5 min after boot)
    sudo tee /etc/systemd/system/idle-monitor.timer > /dev/null <<'EOF'
[Unit]
Description=Private LLM Idle Monitor Timer

[Timer]
OnBootSec=5min
OnUnitActiveSec=1min
Unit=idle-monitor.service
...
EOF
    ...
    sudo systemctl enable idle-monitor.timer
